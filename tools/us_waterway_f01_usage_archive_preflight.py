...
def main():
    ...
    candidates = []
    source_rows = [root_src]
    for pid in ids:
        try:
            j, src = meta(pid); source_rows.append(src)
        except Exception:
            continue
        filename = str(first(j, "find", "filename", "file", "dmrecord") or "")
    ...
    # Known public XLSX fallback; compound metadata may not surface children uniformly.
    ...
    for c in candidates:
        ...
        if not fn.lower().endswith(".xlsx"):
            continue
        url = f"https://usace.contentdm.oclc.org/utils/getfile/collection/{COLL}/filename/{fn}"
        try:
            b, status, final = get(url)
        except Exception as e:
            books.append({**c, "download_url": url, "error": f"{type(e).__name__}: {e}"})
            continue
        rec = {**c, "download_url": url, "status": status, "final_url": final, "bytes": len(b), "sha256": sha256(b)}
        try:
        ...
        years = set(); delay_labels = set(); processing_labels = set(); lock_labels = set(); sheet_names = wb.sheetnames
        for ws in wb.worksheets:
            for row in ws.iter_rows(values_only=True):
                for v in row:
                    ...
                    # lock identity labels/headings only; never numeric outcome values
                    if ("lock" in low or "l/d" in low) and len(s) <= 120:
                        if not any(t in low for t in ["average delay", "processing", "lockage", "unavailability", "percent"]):
                            lock_labels.add(s)
        wb.close()
        yrs = sorted(years)
        contiguous = []
        for i in range(len(yrs)-2):
            if yrs[i+1] == yrs[i]+1 and yrs[i+2] == yrs[i]+2:
                contiguous.append([yrs[i], yrs[i+1], yrs[i+2]])
        rec.update({
            "sheet_names": sheet_names,
            "year_labels_2016_2025": yrs,
            "contiguous_3yr_windows": contiguous,
            "delay_metric_labels": sorted(delay_labels),
            "processing_metric_labels": sorted(processing_labels),
            "lock_identity_label_count": len(lock_labels),
            "lock_identity_label_sample": sorted(lock_labels)[:80],
    ...
    if valid:
        best = max(valid, key=lambda x: (max(x.get("year_labels_2016_2025") or [0]), x.get("lock_identity_label_count", 0)))
    result = {
        "boundary": {"delay_magnitudes_parsed": False, "processing_magnitudes_parsed": False, "hydrology_magnitudes_parsed": False, "relationship_computed": False},
        "compound_id": COMPOUND_ID,
        "compound_source": root_src,
        "candidate_count": len(candidates),
        "workbooks": books,
        "historical_outcome_route_pass": bool(best),
        "best_public_workbook": best,
