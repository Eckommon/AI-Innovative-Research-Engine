#!/usr/bin/env python3
"""Extended outcome-blind USGS metadata qualification for US-WATERWAY-F01.

Same frozen 2018-2020 metadata-only contract as v2; extends the operational stop
from 25 to 35 qualified locks so the historical-usage crosswalk can be tested
without relaxing identity rules. No hydrologic values or delay values are read.
"""
...
UA='AI-Innovative-Research-Engine/US-WATERWAY-F01-fast-v3'
...
START='2018-01-01'; END='2020-12-31'; TARGET=35
...
def main():
    ...
    for lock in locks:
        ...
        for d,mid,name in sorted(cands):
            ...
            if supports:
                ...
                break
        time.sleep(.03)
    result={'boundary':{'delay_magnitudes_parsed':False,'hydrology_values_requested':False,'hydrology_values_parsed':False,'relationship_computed':False,'incremental_monetary_cost_usd':0},'target_overlap':[START,END],'target_qualified_locks':TARGET,'national_lock_features':len(obj.get('features',[])),'pmsdata_y_geometry_locks':len(locks),'locks_attempted':attempts,'qualified_matches':len(matches),'metadata_requests':reqs,'errors':errors[:100],'matches':matches}
    (OUT/'HYDROLOGY_METADATA_PREFLIGHT_V3.json').write_text(json.dumps(result,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
    lines=['# US-WATERWAY-F01 Hydrology Metadata Preflight v3','', 'No delay magnitudes or hydrologic observation values were requested or parsed.','',f'- national lock features: **{len(obj.get("features",[]))}**',f'- `PMSDATA=Y` locks with geometry: **{len(locks)}**',f'- target overlap: **{START}–{END}**',f'- locks attempted before stop: **{attempts}**',f'- qualified lock↔USGS metadata matches: **{len(matches)} / {TARGET}**',f'- metadata requests: **{reqs}**','', 'Qualification is unchanged from v2: coordinate search envelope plus full river/waterbody identity in the USGS monitoring-location name, with Daily `00060`/`00065` metadata covering the full target interval.','', '## Qualified matches']
    for m in matches:
    ...
    (OUT/'HYDROLOGY_METADATA_PREFLIGHT_V3.md').write_text('\n'.join(lines)+'\n',encoding='utf-8')
