...
JSON_OUT = OUTDIR / "attempt-03.json"
MD_OUT = OUTDIR / "attempt-03.md"
        ...
        for r in range(start,end):
            ...
            for c in range(self._sheet.ncols):
                ...
                if cell.ctype==xlrd.XL_CELL_DATE:
                    try:
                        dt=xlrd.xldate.xldate_as_datetime(v,self._datemode)
                        v=dt
                    except Exception:
                        pass
                vals.append(v)
...
class XlrdBookAdapter:
    def __init__(self, book):
        self.worksheets=[XlrdSheetAdapter(book.sheet_by_index(i), book.datemode) for i in range(book.nsheets)]
...
def main():
    ...
    if JSON_OUT.exists() or MD_OUT.exists():
        raise RuntimeError("immutable attempt-03 evidence already exists")
    ev={
        "research":"US-HUD-MF-F01","attempt":3,"contract_sha":CONTRACT_SHA,"issue":ISSUE,
    ...
    ev["attempt_01_commit"]="73ce4cfe7f475bb177186ab7d38ce7374f5e8fee"
    ev["attempt_02_commit"]="0dcd10db85848957ae950b592a13ed83348c1964"
    ev["implementation_correction_02_commit"]="5780fbfe2258d901d615cd23bcba45a71279328c"
    ev["implementation_correction_commit"]="4f59cc6e0b705e2ccbd91f2324a3a99ea70b73b7"
    ev["corrected_runner_base_commit"]="1071033219e9bcc87b92c0067d9844e31c798c68"
    ev["scientific_threshold_changed"]=False
    ev["snapshot_changed"]=False
    ev["identity_rule_changed"]=False
    ev["future_outcome_firewall_changed"]=False
    try:
        ...
        with tempfile.TemporaryDirectory() as td0:
            ...
            if "active" in files:
                ...
                if ws:
                    ...
                    bi=idx_for(norms,("amoritizedprincipalbalance","amortizedprincipalbalance","amortizedunpaidprincipalbalance","unpaidprincipalbalance","upb"))
        ...
        ev["failed_gates"]=[]
        ev["disposition"]="IMPLEMENTATION_BLOCKED_US_HUD_MF_F01_ATTEMPT_03"

    JSON_OUT.write_text(json.dumps(ev,indent=2,sort_keys=True,ensure_ascii=False)+"\n",encoding="utf-8")
    lines=["# US-HUD-MF-F01 — Attempt 03","",f"**Disposition:** `{ev['disposition']}`","",
