...
# Extract the nationwide CSV popup route from the button markup.
...
out={'id':'US-WW-F01-CWNS-NATIONAL-CSV-POPUP-ROUTE','issue':114,'relationship_computed':False,'candidate_outcome_magnitudes_opened':False,'main_fetch':{'url':final,'status':status,'bytes':nbytes},'popup_url':popup,'popup_fetch':{'url':pfinal,'status':pstatus,'bytes':pbytes},'candidate_links':sorted(set(links)),'controls':controls[:100],'snippets':snips[:50],'incremental_monetary_cost_usd':0}
(OUT/'CWNS_NATIONAL_POPUP_ROUTE.json').write_text(json.dumps(out,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
print(json.dumps({'popup_status':pstatus,'links':len(out['candidate_links']),'controls':len(out['controls'])}))
